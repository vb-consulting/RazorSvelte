<script lang="ts">
    ...
    const getCountries = () => get<ICountry[]>(urls.countriesUrl, { "culture-contains": "en", limit: 5 }); 
...
<Layout>
    <div class="container text-center">
        ...
        <DataTable 
            ...
            <tr slot="bottomRow" let:instance>
                <td colspan="999999" class="text-center">
                    <button class="btn btn-primary btn-sm" on:click={() => instance.refresh()}>Load again</button>
                </td>
            </tr>
        </DataTable>
        ...
        <DataTable 
            ...
            small
            readBehavior="custom"
            headers={["Id", "Country", "Code", "Iso2 Code", "Iso3 Code", "Culture"]}>
            <svelte:fragment slot="caption" let:instance>
                Click on this button to load data manually
                <button class="btn btn-primary btn-sm" on:click={() => instance.refresh()}>Load</button>
            </svelte:fragment>
            <tr slot="row" let:index let:data>
                <td>{index + 1}</td>
                <td>{data.name}</td>
